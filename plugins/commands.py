...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Get All Files in a Single Click!!!\n\n📂 ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 5 mins to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('📂 𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗 𝗡𝗢𝗪 📂', url=g)
                    ], [
                        InlineKeyboardButton('💠 𝗛𝗢𝗪 𝗧𝗢 𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗 💠', url=await get_tutorial(chat_id))
                    ]
                ]
            )
        )
        ...
        for file in files:
            ...
            filesarr.append(msg)
        k = await client.send_message(chat_id = message.from_user.id, text=f"<b>❗️❗️This Movie Files/Videos will be deleted in 10 mins</b>.\n<b><i>Please forward this ALL Files/Videos to your Saved Messages and Start Download there</i></b>")
    ...
    if not files_:
        ...
        try:
            ...
            title = '@AmanBotz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@AmanBotz ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
...
async def shortlink(bot, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete :(\n\nGive me a shortener website link and api along with the command !\n\nFormat: <code>/shortlink onepagelink.in b6625e6a99051a5d07302ee94ba54885befb65c5</code></b>")
